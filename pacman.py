...
class GhostRules:
    ...
    def getLegalActions( state, ghostIndex ):
        ...
        # If we have STOP or some other action and reverse then get rid of reverse
        if reverse in possibleActions and len( possibleActions ) > 2:
            possibleActions.remove( reverse )

        # We need STOP for the stationary RL ghost
...
def readCommand( argv ):
    ...
    parser.add_option('-c', '--catchExceptions', action='store_false', dest='catchExceptions',
    ...
    # Prevent user from selecting keyboard agent with no display
    # Choose a Pacman agent
    noKeyboard = options.gameToReplay == None and (options.textGraphics or options.quietGraphics)
    if noKeyboard and "keyboardAgents" in [options.pacman, options.ghost]:
        raise Exception('Using the keyboard requires graphics (not text display)')


    pacmanType = import_by_name("./agents", options.pacman)
    ...
    ghostType = import_by_name("./agents", options.ghost)
...
def runGames( layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30, *args, **kwargs):
    logger = logging.getLogger('root')
    logger.info('runGames')
    ...
    if layout.endswith('.lay'):
        layoutNames = [layout]
    # otherwise get all layout files in the directory
    else:
        layoutNames = [filename for filename in os.listdir(layout) if filename.endswith('.lay')]

    for i in range( numGames ):
        logger.info(str(i)+'/'+str(numGames))
