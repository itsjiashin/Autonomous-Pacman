...
def raiseNotDefined():
    ...
    raise NotImplementedError("*** Method not implemented: %s at line %s of %s" % (method, line, fileName))
...
def import_by_name(directory: str, target_name: str):
    for filename in glob.glob(directory + "/*.py"):
        spec = importlib.util.spec_from_file_location(target_name, filename)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)

        if hasattr(module, target_name):
            print(f"Loading {target_name} from {module}")
            return getattr(module, target_name)

    raise ImportError(f"{target_name} not found in {directory}")
